...
```{r}
PRRS %>% 
  select(Result, Age, Sex) %>% # select COlumns Result, Age, Sex
  filter(Age >= 20) %>% 
  filter(Result == 1)
```
...
PRRS %>% 
  filter(farm_type == 'boar studs') # we will use the equals to operator for this
```
